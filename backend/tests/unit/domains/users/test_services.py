...
class TestUserServiceParseId:
    """Test suite for UserService.parse_id method."""

    def test_parses_integer_ids(
        self, mock_repository: AsyncMock, mock_password_service: AsyncMock
    ) -> None:
        """Test UserService parses integer IDs."""
    ...
    def test_raises_invalid_user_id_error_for_invalid_id(
        self, mock_repository: AsyncMock, mock_password_service: AsyncMock
    ) -> None:
        """Test UserService raises InvalidUserIDError for invalid ID string."""
        from app.domains.users.exceptions import InvalidUserIDError

        service = UserService(mock_repository, mock_password_service)

        with pytest.raises(InvalidUserIDError):
            service.parse_id("not-an-integer")
